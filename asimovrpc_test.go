...
type AsimovRPCTestSuite struct {
	suite.Suite
	rpc *AsimovRPC
}

func (s *AsimovRPCTestSuite) registerResponse(result string, callback func([]byte)) {
	httpmock.Reset()
...
func (s *AsimovRPCTestSuite) registerResponseError(err error) {
	httpmock.Reset()
	httpmock.RegisterResponder("POST", s.rpc.url, func(request *http.Request) (*http.Response, error) {
		return nil, err
	})
...
func (s *AsimovRPCTestSuite) getBody(request *http.Request) []byte {
...
func (s *AsimovRPCTestSuite) methodEqual(body []byte, expected string) {
	value := gjson.GetBytes(body, "method").String()

	s.Require().Equal(expected, value)
}

func (s *AsimovRPCTestSuite) paramsEqual(body []byte, expected string) {
...
func (s *AsimovRPCTestSuite) SetupSuite() {
	s.rpc = NewAsimovRPC("https://test-rpc.asimov.network", WithHttpClient(http.DefaultClient), WithLogger(nil), WithDebug(false))

	httpmock.Activate()
}

func (s *AsimovRPCTestSuite) TearDownSuite() {
	httpmock.Deactivate()
}

func (s *AsimovRPCTestSuite) TearDownTest() {
	httpmock.Reset()
}

func (s *AsimovRPCTestSuite) TestURL() {
	s.Require().Equal(s.rpc.url, s.rpc.URL())
}

func (s *AsimovRPCTestSuite) TestWeb3ClientVersion() {
...
func (s *AsimovRPCTestSuite) TestCall() {
	// Test http error
	httpmock.RegisterResponder("POST", s.rpc.url, func(request *http.Request) (*http.Response, error) {
	...
	_, err = s.rpc.Call("test")
	s.Require().NotNil(err)
	AsimovError, ok := err.(AsimovError)
	s.Require().True(ok)
	s.Require().Equal(21, AsimovError.Code)
	s.Require().Equal("eee", AsimovError.Message)
}

func (s *AsimovRPCTestSuite) Test_call() {
	// Test http error
	httpmock.RegisterResponder("POST", s.rpc.url, func(request *http.Request) (*http.Response, error) {
...
func (s *AsimovRPCTestSuite) TestWeb3Sha3() {
...
func (s *AsimovRPCTestSuite) TestNetVersion() {
...
func (s *AsimovRPCTestSuite) TestNetListening() {
...
func (s *AsimovRPCTestSuite) TestNetPeerCount() {
	// Test error
...
func (s *AsimovRPCTestSuite) TestAsimovProtocolVersion() {
	s.registerResponse(`"54"`, func(body []byte) {
		s.methodEqual(body, "flow_protocolVersion")
		s.paramsEqual(body, "null")
	})

	protocolVersion, err := s.rpc.AsimovProtocolVersion()
	s.Require().Nil(err)
	s.Require().Equal("54", protocolVersion)
}

func (s *AsimovRPCTestSuite) TestAsimovSyncing() {
	s.registerResponseError(errors.New("Error"))
	syncing, err := s.rpc.AsimovSyncing()
	s.Require().NotNil(err)
	...
	s.registerResponse(`false`, func(body []byte) {
		s.methodEqual(body, "flow_syncing")
	})
	syncing, err = s.rpc.AsimovSyncing()
...
func (s *AsimovRPCTestSuite) TestAsimovCoinbase() {
	s.registerResponse(`"0x407d73d8a49eeb85d32cf465507dd71d507100c1"`, func(body []byte) {
		s.methodEqual(body, "flow_coinbase")
		s.paramsEqual(body, "null")
	})

	address, err := s.rpc.AsimovCoinbase()
	s.Require().Nil(err)
	s.Require().Equal("0x407d73d8a49eeb85d32cf465507dd71d507100c1", address)
}
func (s *AsimovRPCTestSuite) TestAsimovMining() {
	s.registerResponse(`true`, func(body []byte) {
		s.methodEqual(body, "flow_mining")
		s.paramsEqual(body, "null")
	})

	mining, err := s.rpc.AsimovMining()
	s.Require().Nil(err)
...
func (s *AsimovRPCTestSuite) TestAsimovHashrate() {
	s.registerResponseError(errors.New("Error"))
	hashrate, err := s.rpc.AsimovHashrate()
	s.Require().NotNil(err)

	s.registerResponse(`"0x38a"`, func(body []byte) {
		s.methodEqual(body, "flow_hashrate")
		s.paramsEqual(body, "null")
	})
...
func (s *AsimovRPCTestSuite) TestAsimovGasPrice() {
	s.registerResponseError(errors.New("Error"))
	gasPrice, err := s.rpc.AsimovGasPrice()
	s.Require().NotNil(err)

	s.registerResponse(`"0x09184e72a000"`, func(body []byte) {
		s.methodEqual(body, "flow_gasPrice")
		s.paramsEqual(body, "null")
	})
...
func (s *AsimovRPCTestSuite) TestAsimovAccounts() {
	s.registerResponse(`["0x407d73d8a49eeb85d32cf465507dd71d507100c1"]`, func(body []byte) {
		s.methodEqual(body, "flow_accounts")
		s.paramsEqual(body, "null")
	})

	accounts, err := s.rpc.AsimovAccounts()
	s.Require().Nil(err)
	s.Require().Equal([]string{"0x407d73d8a49eeb85d32cf465507dd71d507100c1"}, accounts)
}

func (s *AsimovRPCTestSuite) TestAsimovBlockNumber() {
	s.registerResponseError(errors.New("Error"))
	blockBumber, err := s.rpc.AsimovBlockNumber()
	s.Require().NotNil(err)

	s.registerResponse(`"0x37eb38"`, func(body []byte) {
		s.methodEqual(body, "flow_blockNumber")
		s.paramsEqual(body, "null")
	})
...
func (s *AsimovRPCTestSuite) TestAsimovGetBalance() {
	...
	s.registerResponse(`"0x486d06b0d08d05909c4"`, func(body []byte) {
		s.methodEqual(body, "flow_getBalance")
...
func (s *AsimovRPCTestSuite) TestAsimovGetStorageAt() {
	...
	s.registerResponse(`"0x00000000000000000000000000000000000000000000000000000000000004d2"`, func(body []byte) {
		s.methodEqual(body, "flow_getStorageAt")
		s.paramsEqual(body, fmt.Sprintf(`["%s", "0x21", "pending"]`, data))
	})

	result, err := s.rpc.AsimovGetStorageAt(data, position, tag)
	s.Require().Nil(err)
	s.Require().Equal("0x00000000000000000000000000000000000000000000000000000000000004d2", result)
}

func (s *AsimovRPCTestSuite) TestAsimovGetTransactionCount() {
	address := "0x407d73d8a49eeb85d32cf465507dd71d507100c1"
	s.registerResponseError(errors.New("Error"))
	count, err := s.rpc.AsimovGetTransactionCount(address, "latest")
	s.Require().NotNil(err)

	s.registerResponse(`"0x10"`, func(body []byte) {
		s.methodEqual(body, "flow_getTransactionCount")
		s.paramsEqual(body, fmt.Sprintf(`["%s", "latest"]`, address))
	})
...
func (s *AsimovRPCTestSuite) TestAsimovGetBlockTransactionCountByHash() {
	hash := "0xb903239f8543d04b5dc1ba6579132b143087c68db1b2168786408fcbce568238"
	s.registerResponseError(errors.New("Error"))
	count, err := s.rpc.AsimovGetBlockTransactionCountByHash(hash)
	s.Require().NotNil(err)

	s.registerResponse(`"0xb"`, func(body []byte) {
		s.methodEqual(body, "flow_getBlockTransactionCountByHash")
...
func (s *AsimovRPCTestSuite) TestAsimovGetBlockTransactionCountByNumber() {
	number := 2384732
	s.registerResponseError(errors.New("Error"))
	count, err := s.rpc.AsimovGetBlockTransactionCountByNumber(number)
	s.Require().NotNil(err)

	s.registerResponse(`"0xe8"`, func(body []byte) {
		s.methodEqual(body, "flow_getBlockTransactionCountByNumber")
...
func (s *AsimovRPCTestSuite) TestAsimovGetUncleCountByBlockHash() {
	hash := "0xb903239f8543d04b5dc1ba6579132b143087c68db1b2168786408fcbce568238"
	s.registerResponseError(errors.New("Error"))
	count, err := s.rpc.AsimovGetUncleCountByBlockHash(hash)
	s.Require().NotNil(err)

	s.registerResponse(`"0xa"`, func(body []byte) {
		s.methodEqual(body, "flow_getUncleCountByBlockHash")
		s.paramsEqual(body, fmt.Sprintf(`["%s"]`, hash))
	})
...
func (s *AsimovRPCTestSuite) TestAsimovGetUncleCountByBlockNumber() {
	number := 3987434
	s.registerResponseError(errors.New("Error"))
	count, err := s.rpc.AsimovGetUncleCountByBlockNumber(number)
	s.Require().NotNil(err)

	s.registerResponse(`"0x386"`, func(body []byte) {
		s.methodEqual(body, "flow_getUncleCountByBlockNumber")
...
func (s *AsimovRPCTestSuite) TestAsimovGetCode() {
	address := "0xa94f5374fce5edbc8e2a8697c15331677e6ebf0b"
	result := "0x600160008035811a818181146012578301005b601b6001356025565b8060005260206000f25b600060078202905091905056"
	s.registerResponse(fmt.Sprintf(`"%s"`, result), func(body []byte) {
		s.methodEqual(body, "flow_getCode")
		s.paramsEqual(body, fmt.Sprintf(`["%s", "latest"]`, address))
	})

	code, err := s.rpc.AsimovGetCode(address, "latest")
	s.Require().Nil(err)
	s.Require().Equal(result, code)
}

func (s *AsimovRPCTestSuite) TestAsimovSign() {
	address := "0x9b2055d370f73ec7d8a03e965129118dc8f5bf83"
	data := "0xdeadbeaf"
	result := "0xa3f20717a250c2b0b729b7e5becbff67fdaef7e0699da4de7ca5895b02a170a12d887fd3b17bfdce3481f10bea41f45ba9f709d39ce8325427b57afcfc994cee1b"
	s.registerResponse(fmt.Sprintf(`"%s"`, result), func(body []byte) {
		s.methodEqual(body, "flow_sign")
		s.paramsEqual(body, fmt.Sprintf(`["%s", "%s"]`, address, data))
	})

	signed, err := s.rpc.AsimovSign(address, data)
	s.Require().Nil(err)
	s.Require().Equal(result, signed)
}

func (s *AsimovRPCTestSuite) TestSendTransaction() {
	t := T{
	...
	result := "0xea1115eb5"
	s.registerResponse(fmt.Sprintf(`"%s"`, result), func(body []byte) {
		s.methodEqual(body, "flow_sendTransaction")
		s.paramsEqual(body, `[{
			"from": "0x3cc1a3c082944b9dba70e490e481dd56",
			"to": "0x1bf21cb1dc384d019a885a06973f7308",
			"gas": "0x6144",
	...
	txid, err := s.rpc.AsimovSendTransaction(t)
	s.Require().Nil(err)
...
func (s *AsimovRPCTestSuite) TestAsimovSendRawTransaction() {
	data := "0xd46e8dd67c5d32be8d46e8dd67c5d32be8058bb8eb970870f072445675058bb8eb970870f072445675"
	result := "0xe670ec64341771606e55d6b4ca35a1a6b75ee3d5145a99d05921026d1527331"
	s.registerResponse(fmt.Sprintf(`"%s"`, result), func(body []byte) {
		s.methodEqual(body, "flow_sendRawTransaction")
		s.paramsEqual(body, fmt.Sprintf(`["%s"]`, data))
	})

	txid, err := s.rpc.AsimovSendRawTransaction(data)
	s.Require().Nil(err)
	s.Require().Equal(result, txid)
}

func (s *AsimovRPCTestSuite) TestAsimovGetCompilers() {
	s.registerResponse(`["solidity", "some comp"]`, func(body []byte) {
		s.methodEqual(body, "flow_getCompilers")
		s.paramsEqual(body, "null")
	})

	compilers, err := s.rpc.AsimovGetCompilers()
	s.Require().Nil(err)
	s.Require().Equal([]string{"solidity", "some comp"}, compilers)

}

func (s *AsimovRPCTestSuite) TestGetBlock() {
	s.registerResponseError(errors.New("Error"))
	block, err := s.rpc.getBlock("flow_getBlockByHash", true)
	s.Require().NotNil(err)
	...
	s.registerResponse(result, func(body []byte) {
		s.methodEqual(body, "flow_getBlockByHash")
	})
	...
	block, err = s.rpc.getBlock("flow_getBlockByHash", false)
	s.Require().Nil(err)
	s.Require().NotNil(block)
	s.Require().Equal(hash, block.Hash)
...
func (s *AsimovRPCTestSuite) TestAsimovGetBlockByHash() {
	// Test with transactions
	hash := "0x111"
	...
	_, err := s.rpc.AsimovGetBlockByHash(hash, true)
	s.Require().Nil(err)
	...
	_, err = s.rpc.AsimovGetBlockByHash(hash, false)
	s.Require().Nil(err)
}

func (s *AsimovRPCTestSuite) TestAsimovGetBlockByNumber() {
	// Test with transactions
	number := 3274863
	s.registerResponse(`{}`, func(body []byte) {
		s.methodEqual(body, "flow_getBlockByNumber")
		s.paramsEqual(body, `["0x31f86f", true]`)
	})

	_, err := s.rpc.AsimovGetBlockByNumber(number, true)
	s.Require().Nil(err)
	...
	_, err = s.rpc.AsimovGetBlockByNumber(number, false)
	s.Require().Nil(err)
}

func (s *AsimovRPCTestSuite) TestAsimovCall() {
	s.registerResponse(`"0x11"`, func(body []byte) {
		s.methodEqual(body, "flow_call")
		s.paramsEqual(body, `[{"from":"0x111","to":"0x222"}, "ttt"]`)
	})

	result, err := s.rpc.AsimovCall(T{
		From: "0x111",
		To:   "0x222",
	}, "ttt")
	s.Require().Nil(err)
...
func (s *AsimovRPCTestSuite) TestAsimovEstimateGas() {
	s.registerResponseError(errors.New("error"))
	result, err := s.rpc.AsimovEstimateGas(T{
		From: "0x111",
		To:   "0x222",
	})
	s.Require().NotNil(err)
	...
	s.registerResponse(`"0x5022"`, func(body []byte) {
		s.methodEqual(body, "flow_estimateGas")
...
func (s *AsimovRPCTestSuite) TestAsimovGetTransactionReceipt() {
	hash := "0x9c17afa5336d3cfd47e2e795520959b92e627e123e538fd4d5d7ece9025a8dce"
	s.registerResponseError(errors.New("error"))
	receipt, err := s.rpc.AsimovGetTransactionReceipt(hash)
	s.Require().NotNil(err)
	...
	s.registerResponse(result, func(body []byte) {
		s.methodEqual(body, "flow_getTransactionReceipt")
...
func (s *AsimovRPCTestSuite) TestGetTransaction() {
	result := `{
...
func (s *AsimovRPCTestSuite) TestAsimovGetTransactionByHash() {
	s.registerResponse(`{}`, func(body []byte) {
		s.methodEqual(body, "flow_getTransactionByHash")
		s.paramsEqual(body, `["0x123"]`)
	})

	t, err := s.rpc.AsimovGetTransactionByHash("0x123")
	s.Require().Nil(err)
	s.Require().NotNil(t)
}

func (s *AsimovRPCTestSuite) TestAsimovGetTransactionByBlockHashAndIndex() {
	s.registerResponse(`{}`, func(body []byte) {
		s.methodEqual(body, "flow_getTransactionByBlockHashAndIndex")
		s.paramsEqual(body, `["0x623", "0x12"]`)
	})

	t, err := s.rpc.AsimovGetTransactionByBlockHashAndIndex("0x623", 18)
	s.Require().Nil(err)
	s.Require().NotNil(t)
}

func (s *AsimovRPCTestSuite) TestAsimovGetTransactionByBlockNumberAndIndex() {
	s.registerResponse(`{}`, func(body []byte) {
		s.methodEqual(body, "flow_getTransactionByBlockNumberAndIndex")
		s.paramsEqual(body, `["0x1f537da", "0xa"]`)
	})

	t, err := s.rpc.AsimovGetTransactionByBlockNumberAndIndex(32847834, 10)
	s.Require().Nil(err)
	s.Require().NotNil(t)
}

func (s *AsimovRPCTestSuite) TestAsimovNewFilterWithAddress() {
	...
	s.registerResponse(fmt.Sprintf(`"%s"`, result), func(body []byte) {
		s.methodEqual(body, "flow_newFilter")
		s.paramsEqual(body, fmt.Sprintf(`[{"address": ["%s"]}]`, address[0]))
	})

	filterID, err := s.rpc.AsimovNewFilter(filterData)
	s.Require().Nil(err)
	s.Require().Equal(result, filterID)
}

func (s *AsimovRPCTestSuite) TestAsimovNewFilterWithTopics() {
	topics := [][]string{
		{
...
func (s *AsimovRPCTestSuite) TestAsimovNewFilterWithAddressAndTopics() {
	topics := [][]string{
		{"0xb2b2eeeee341e560da3d439ef5e5309d78a22a66"},
		{"0xb2b2fffff341e560da3d439ef5e5309d78a22a66"},
	}
...
func (s *AsimovRPCTestSuite) TestAsimovNewBlockFilter() {
	result := "0x6996a3a4788d4f2067108d1f536d4330"
	s.registerResponse(fmt.Sprintf(`"%s"`, result), func(body []byte) {
		s.methodEqual(body, "flow_newBlockFilter")
	})

	filterID, err := s.rpc.AsimovNewBlockFilter()
	s.Require().Nil(err)
	s.Require().Equal(result, filterID)
}

func (s *AsimovRPCTestSuite) TestAsimovNewPendingTransactionFilter() {
	result := "0x153"
	s.registerResponse(fmt.Sprintf(`"%s"`, result), func(body []byte) {
		s.methodEqual(body, "flow_newPendingTransactionFilter")
	})

	filterID, err := s.rpc.AsimovNewPendingTransactionFilter()
	s.Require().Nil(err)
	s.Require().Equal(result, filterID)
}

func (s *AsimovRPCTestSuite) TestAsimovGetFilterChanges() {
	...
	s.registerResponse(result, func(body []byte) {
		s.methodEqual(body, "flow_getFilterChanges")
		s.paramsEqual(body, fmt.Sprintf(`["%s"]`, filterID))
	})

	logs, err := s.rpc.AsimovGetFilterChanges(filterID)
	s.Require().Nil(err)
	s.Require().Equal([]Log{
		{
			Address:     "0xaca0cc3a6bf9552f2866ccc67801d4e6aa6a70f2",
...
func (s *AsimovRPCTestSuite) TestAsimovGetFilterLogs() {
	filterID := "0x6996a3a4788d4f2067108d1f536d4330"
	result := `[{
		"address": "0xaca0cc3a6bf9552f2866ccc67801d4e6aa6a70f2",
		"blockHash": "0x9d9838090bb7f6194f62acea788688435b79cc44c62dcf1479abd9f2c72a7d5c",
	...
	s.registerResponse(result, func(body []byte) {
		s.methodEqual(body, "flow_getFilterLogs")
		s.paramsEqual(body, fmt.Sprintf(`["%s"]`, filterID))
	})

	logs, err := s.rpc.AsimovGetFilterLogs(filterID)
	s.Require().Nil(err)
	s.Require().Equal([]Log{
		{
			Address:     "0xaca0cc3a6bf9552f2866ccc67801d4e6aa6a70f2",
...
func (s *AsimovRPCTestSuite) TestAsimovGetLogs() {
	params := FilterParams{
		FromBlock: "0x1",
		ToBlock:   "0x10",
	...
	s.registerResponse(result, func(body []byte) {
		s.methodEqual(body, "flow_getLogs")
	...
	logs, err := s.rpc.AsimovGetLogs(params)
	s.Require().Nil(err)
	s.Require().Equal([]Log{
		{
			Address:     "0xaca0cc3a6bf9552f2866ccc67801d4e6aa6a70f2",
...
func (s *AsimovRPCTestSuite) TestAsimovUninstallFilter() {
	filterID := "0x6996a3a4788d4f2067108d1f536d4330"
	result := "true"
	s.registerResponse(result, func(body []byte) {
		s.methodEqual(body, "flow_uninstallFilter")
		s.paramsEqual(body, fmt.Sprintf(`["%s"]`, filterID))
	})

	uninstall, err := s.rpc.AsimovUninstallFilter(filterID)
	s.Require().Nil(err)
	boolRes, _ := strconv.ParseBool(result)
	s.Require().Equal(boolRes, uninstall)
}

func TestAsimovRPCTestSuite(t *testing.T) {
	suite.Run(t, new(AsimovRPCTestSuite))
}

func TestAsimovError(t *testing.T) {
	var err error
	err = AsimovError{-32555, "Messg"}
	require.Equal(t, "Error -32555 (Messg)", err.Error())

	err = AsimovError{32847, "Kuku"}
	require.Equal(t, "Error 32847 (Kuku)", err.Error())
}

func TestAsimov1(t *testing.T) {
	client := NewAsimovRPC("")
	require.Equal(t, int64(1000000000000000000), Asim1().Int64())
	require.Equal(t, int64(1000000000000000000), client.Asim1().Int64())
}
